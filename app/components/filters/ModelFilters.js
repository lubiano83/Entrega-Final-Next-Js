            ...
            <div className={`absolute mt-2 z-10 w-36 rounded-md max-h-72 overflow-y-scroll shadow-lg ${isDarkMode ? "bg-orange-400" : "bg-blue-400"} ring-1 ring-black ring-opacity-5`}>
              <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                ...
                <Link href={`/pages/products/${category}/${brand}`}>
                  <span onClick={closeDropdowns} className={`${(category === "todos" && brand === "todos" ? "text-gray-700 flex justify-center items-center text-sm px-2 text-center" : "block px-4 py-2 text-sm text-white hover:bg-gray-700")} ${path === `/pages/products/${category}/${brand}` ? "underline" : "no-underline"}`} role="menuitem">
