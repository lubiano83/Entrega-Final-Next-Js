...
const HigherPrice = async () => {
  try {
    const items = await fetch(`${process.env.NEXT_PUBLIC_FIREBASE_API_URL}/products?limit=4&sort=desc`, {next: {revalidate: 3600, tags: ['products', 'product', 'cart']}}).then(res => res.json());
